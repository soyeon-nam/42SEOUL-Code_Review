...
/*   Updated: 2021/06/13 15:57:11 by sshin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//tmp free를 한번으로 가능한데 중복으로 쓰여서 줄이 길어짐. (임의 수정) -jichung
{
	//0보다 크다는 것보다 _LF_NOT_FOUND가 아닐때가 더 명확하게 이해된다고 생각함. (임의 수정) -jichung
	//buf free를 한번으로 가능한데 중복으로 쓰여서 줄이 길어짐. (임의 수정) -jichung
	char *buf;
	char *tmp;
...
//if와 else의 작성순서를 바꿨으면 free가 더욱 용이해졌으리라 생각됨. 그랬으면 노미넷을 지키기가 더 수월했을것. (임의 수정) -jichung
{
	char *tmp;
...
{
	if (read_size < 0)
	//read 함수 결과값의 예외처리가 이 위치에 있는게 상당히 어색함 -jichung
		return (_ERROR);
	if (*backup == NULL)
	{
		*line = ft_strdup("");
...
//전반적으로 아주 잘 작성됨. 코드의 진행이 아주 심플한 점이 특히 인상깊었음.
// 그런데 노미넷 규정을 free를 중복으로 작성하여 어김으로서 일부 불필요하게 복잡해진 부분이 존재함.
// 그 이외에는 모두 완벽. -jichung